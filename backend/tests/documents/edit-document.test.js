...
async function testEditDocument() {
  if (!filePath) {
    console.error('Usage: node tests/documents/edit-document.test.js <path_to_file>');
    console.error('Example: node tests/documents/edit-document.test.js ~/Downloads/sample.png');
    process.exit(1);
  }
  ...
  try {
    ...
    const editResponse = await fetch(`${API_DOCUMENTS_URL}/${documentId}`, {
      method: 'PATCH',
    ...
    const immutableResponse = await fetch(`${API_DOCUMENTS_URL}/${documentId}`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${idToken}`, 'Content-Type': 'application/json' },
